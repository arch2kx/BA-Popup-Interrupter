interface Window {
    __baPopupInterrupterReady?: boolean;
}
...
(() => {
    if (window.__baPopupInterrupterReady) return;
    ...
    function showPopup(image: string, duration: number, size: number): void {
        const margin = 16;
    ...
    chrome.runtime.onMessage.addListener((message: ShowPopupMessage) => {
        if (message?.type !== "show-popup") return;
